...
class TrainerMultiTower():
    ...
    def __init__(self,
        ...
        with tf.device('/cpu:0'):
            ...
            def computer_tower_grads(grads_and_vars):
                grads, vs = zip(*average_gradients(grads_and_vars))
                # trust_radius = 1e-4
                ...
                for g_idx, g in enumerate(grads):
                    with tf.device('/cpu:'+str(g_idx % 8)):
                        print(g.name, g.shape)
                        self.tower_norms.append(tf.norm(g)/np.prod(g.shape.as_list()))
            ...
            self.train_op = computer_tower_grads(self.tower_grads)
            self.train_op_forces = computer_tower_grads(self.tower_force_grads)
